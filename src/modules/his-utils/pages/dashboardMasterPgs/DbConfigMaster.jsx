...
const DbConfigMaster = () => {
  ...
  const [isSoftDbConnReq, setIsSoftDbConnReq] = useState('No');
  ...
  const handleSaveConfig = async () => {
    ...
    if (isSoftDbConnReq === 'No' && isDbConnReq === '1' && !values?.driverClass?.trim()) {
      setErrors(prev => ({ ...prev, 'driverClassErr': "driver class is required" }));
      isValid = false;
    }
    if (isSoftDbConnReq === 'No' && isDbConnReq === '1' && !values?.connectionURL?.trim()) {
      setErrors(prev => ({ ...prev, 'connectionURLErr': "connection url is required" }));
      isValid = false;
    }
    if (isSoftDbConnReq === 'No' && isDbConnReq === '1' && !values?.userName?.trim()) {
      setErrors(prev => ({ ...prev, 'userNameErr': "user name is required" }));
      isValid = false;
    }
    if (isSoftDbConnReq === 'No' && isDbConnReq === '1' && !values?.password?.trim()) {
      setErrors(prev => ({ ...prev, 'passwordErr': "password is required" }));
      isValid = false;
    }
    if (isSoftDbConnReq === 'No' && isDbConnReq === '0' && !values?.jndiServer?.trim()) {
      setErrors(prev => ({ ...prev, 'jndiServerErr': "primary server is required" }));
      isValid = false;
    }
    if (isSoftDbConnReq === 'No' && isDbConnReq === '0' && !values?.jndiServer1?.trim()) {
        ...
        <div className='form-card m-auto p-2'>
          ...
          <div className='py-2 px-2'>
            ...
            <div className='row role-theme user-form' style={{ paddingBottom: "1px" }}>
              ...
              <div className='col-sm-6'>
                <div className="form-group row">
                  <label className="col-sm-5 col-form-label pe-0 fw-bold">
                    {dt("Is Soft DB Connection Required")}:
                  </label>
                  <div className="col-sm-7 ps-0 align-content-center">
                    <div className="form-check form-check-inline">
                      <input
                        ...
                        value={isSoftDbConnReq}
                        onChange={(e) => setIsSoftDbConnReq('Yes')}
                        checked={isSoftDbConnReq === "Yes"}
                      />
                    ...
                    <div className="form-check form-check-inline">
                      <input
                        ...
                        value={isSoftDbConnReq}
                        onChange={(e) => setIsSoftDbConnReq('No')}
                        checked={isSoftDbConnReq === 'No'}
                      />
